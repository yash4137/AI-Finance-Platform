...
const ColumnMappingStep = ({
  ...
  return <div className="space-y-6">
      ...
      <div className="border rounded-md overflow-y-auto">
        <Table>
          ...
          <TableBody>
            {csvColumns.map((column) => <TableRow
    key={column.id}
  ...
  />
    ...
    const isDisabled = attr.fieldName !== "Skip" && attr.fieldName !== mappings[column.name] && Object.values(mappings).includes(attr.fieldName);
    return <SelectItem
      key={attr.fieldName}
    ...
    >
                      ...
                      {errors[column.name] && <p className="text-[10px] text-red-500">
                          {errors[column.name]}
                        </p>}
                    </div>
                  </div>
...
var stdin_default = ColumnMappingStep;
export {
