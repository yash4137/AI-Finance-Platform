...
const transactionSchema = z.object({
  ...
  paymentMethod: z.union([
    ...
    z.enum(
      ...
      {
        errorMap: (issue) => ({
          message: issue.code === "invalid_enum_value" ? `Payment method must be one of: ${Object.values(PAYMENT_METHODS_ENUM).join(", ")}` : "Invalid payment method"
        })
      }
    )
  ]).transform((val) => val === "" ? void 0 : val).optional()
});
const ConfirmationStep = ({
  ...
  const handleImport = () => {
    ...
    bulkImportTransaction(payload).unwrap().then(() => {
      updateProgress(100);
  ...
  const getAssignFieldToMappedTransactions = () => {
    ...
    csvData.forEach((row, index) => {
      ...
      Object.entries(mappings).forEach(([csvColumn, transactionField]) => {
        if (transactionField === "Skip" || row[csvColumn] === void 0) return;
        transaction[transactionField] = transactionField === "amount" ? Number(row[csvColumn]) : transactionField === "date" ? new Date(row[csvColumn]) : row[csvColumn];
      });
      try {
        ...
        const message = error instanceof z.ZodError ? error.errors.map((e) => {
          if (e.path[0] === "type")
  ...
  >
            ...
            <div className="space-y-1 p-2">
              {Object.entries(errors).map(([row, msg]) => <details key={row} className="group">
...
var stdin_default = ConfirmationStep;
export {
