...
const TransactionForm = (props) => {
  ...
  const handleScanComplete = (data2) => {
    form.reset({
      ...form.getValues(),
      title: data2.title || "",
      amount: data2.amount.toString(),
      type: data2.type || _TRANSACTION_TYPE.EXPENSE,
      category: data2.category?.toLowerCase() || "",
      date: new Date(data2.date),
      paymentMethod: data2.paymentMethod || "",
      isRecurring: false,
      frequency: null,
      description: data2.description || "",
      receiptUrl: data2.receiptUrl || ""
    });
  };
  const onSubmit = (values) => {
    ...
    if (isEdit && transactionId) {
      updateTransaction({ id: transactionId, transaction: payload }).unwrap().then(() => {
        onCloseDrawer?.();
    ...
    createTransaction(payload).unwrap().then(() => {
      form.reset();
      onCloseDrawer?.();
  ...
  return <div className="relative pb-10 pt-5 px-2.5">
      <Form {...form}>
  ...
  />
    ...
    render={({ field }) => <FormItem>
                  ...
                  <SingleSelector
      value={CATEGORIES.find((opt) => opt.value === field.value) || field.value ? { value: field.value, label: field.value } : void 0}
  ...
  />
    ...
    >
      ...
      disabled={(date) => date < /* @__PURE__ */ new Date("2023-01-01")}
      initialFocus
    />
                    </PopoverContent>
                  </Popover>
  ...
  />
    ...
    control={form.control}
    name="isRecurring"
    render={({ field }) => <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
  ...
  />
            ...
            {form.watch("isRecurring") && form.getValues().isRecurring && <FormField
    control={form.control}
    name="frequency"
    render={({ field }) => <FormItem className="recurring-control">
                    <FormLabel>Frequency</FormLabel>
                    <Select
      onValueChange={field.onChange}
      defaultValue={field.value ?? void 0}
      disabled={isScanning}
    >
                      <FormControl className="w-full">
                        <SelectTrigger>
    ...
    />
                      ...
                      <SelectContent>
                        {frequencyOptions.map(({ value, label }) => <SelectItem
      key={value}
      value={value}
      className="!capitalize"
    >
  ...
  >
          ...
          {isLoading && <div className="absolute top-0 left-0 right-0 bottom-0 bg-white/70 dark:bg-background/70 z-50 flex justify-center">
...
var stdin_default = TransactionForm;
export {
