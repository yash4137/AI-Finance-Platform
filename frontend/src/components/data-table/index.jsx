...
function DataTable({
  ...
  const table = useReactTable({
    ...
    onRowSelectionChange: selection ? setRowSelection : void 0,
  ...
  return <div className="w-full">
    ...
    className="max-w-sm"
  />}
          {filters.map(({ key, label, options }) => <Select
    key={key}
  ...
  >
          ...
          {(searchTerm || Object.keys(rowSelection).length > 0 || Object.keys(filterValues).length > 0) && <Button
    variant="ghost"
  ...
  >
        ...
        {selection && hasSelections || isBulkDeleting ? <Button
    disabled={isLoading || isBulkDeleting}
    variant="destructive"
    size="sm"
  ...
  >
    ...
    /* Table */
      ...
      <div className={cn("rounded-md border overflow-x-auto", className)}>
        {isLoading ? <TableSkeleton columns={6} rows={20} /> : <Table
    className={cn(
  ...
  >
            <TableHeader className="sticky top-0 bg-muted z-10 ">
              {table.getHeaderGroups().map((group) => <TableRow key={group.id}>
                  {group.headers.map((header) => <TableHead
    key={header.id}
  ...
  >
    ...
    header.getContext()
            ...
            <TableBody>
              {table.getRowModel().rows.length > 0 ? table.getRowModel().rows.map((row) => <TableRow
    key={row.id}
    data-state={row.getIsSelected() && "selected"}
  >
                    {row.getVisibleCells().map((cell) => <TableCell key={cell.id} className="!text-[13.3px]">
                        {flexRender(
  ...
  >
    ...
    /* Pagination */
      ...
      {isShowPagination && <div className="mt-4">
          <DataTablePagination
